Shader "Sanrio/Chromakey/ChromakeyMultiSampling"
{
    Properties
    {
	    [Header(Debug)]
		...
		SubShader
		{
			...
			void surf (Input i, inout SurfaceOutputStandard o)
			{
				...
				float2 appendResult13 = (float2(( ( i.uv_MainTex.x + sin( ( ( i.uv_MainTex.y * 50.0 ) * (_LerpTime * _LerpTimeScale) ) ) ) + ( ( lerpResult10_g1 * 0.4 ) * (_LerpTime * _LerpTimeScale) ) ) , i.uv_MainTex.y));
				float2 PulseUV = fmod(appendResult13,float2(1,1));
				PulseUV.x += Grain;

				float4 color = tex2D(_MainTex, PulseUV);

				float2 pixelWidth = float2(1.0 / _MainTex_TexelSize.z, 0);
				float2 pixelHeight = float2(0, 1.0 / _MainTex_TexelSize.w);

				// Unfeathered mask
				float mask = maskedTex2D(_MainTex, PulseUV);
				float c = mask;
				float r = maskedTex2D(_MainTex, PulseUV + pixelWidth);
				float l = maskedTex2D(_MainTex, PulseUV - pixelWidth);
				float d = maskedTex2D(_MainTex, PulseUV + pixelHeight); 
				float u = maskedTex2D(_MainTex, PulseUV - pixelHeight);
